...
func backupGitignore() error {
	...
	if err != nil {
		return fmt.Errorf("erreur lors de la sauvegarde du fichier .gitignore: %w", err)
	}
	fmt.Println("Fichier .gitignore sauvegardé sous", backupFile)
	return nil
}
...
func openGitignoreFile(append bool) (*os.File, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Erreur lors de la création du fichier: %w", err)
	}

	return file, nil
}

func generateGitignore(languages []Langage, params params.Params) {
...
func DetecteLanguages(params params.Params) {
	...
	err := filepath.Walk(".", func(path string, info os.FileInfo, err error) error {
		...
		if !info.IsDir() {
			...
			for _, ingorePath := range params.Ignore {
				if ext == ingorePath {
					println("==Ignoring: ", path)
					return nil
				}
...
func GenerateGitignoreFromExtensions(extensions []string, params params.Params) {
	var langages []Langage
